...
# Run 'Before Experiment' code from setupvars
totalwon = 0.0
...
expName = 'practice-card-task-brief-money'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'session': '001',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/Bhanji/Library/CloudStorage/Box-Box/laptop/CardTask/card-task-brief/practice-card-task-brief-money_lastrun.py',
...
Inst1 = visual.TextStim(win=win, name='Inst1',
    text='In this part of the study you will play a card guessing game to win money that will be added to your bonus. \n\nPress ‘1’ to continue.',
    font='Arial',
...
Finished = visual.TextStim(win=win, name='Finished',
    text="That is the end of the practice\n\nWhen you play the game your total amount won will be displayed on this screen.\n\nPlease wait for further instructions.",
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
while continueRoutine:
    ...
    if Inst1_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'Inst1_3.started')
...
blocks = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('design/practice-card-task-brief-blocks.csv'),
...
for thisBlock in blocks:
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        if text_block.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_block.started')
            text_block.setAutoDraw(True)
        if text_block.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_block.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                text_block.tStop = t  # not accounting for scr refresh
                text_block.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_block.stopped')
        ...
        if text_blocktype.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_blocktype.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *fixation* updates
        if fixation.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
            # keep track of start time/frame for later
    ...
    innertrials = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('design/practice-card-task-brief-trials.csv'),
